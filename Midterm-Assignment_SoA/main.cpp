...
int main() {
    ...
    int nRepeat = 100;
    TimeSeriesData timeseriesData, timeseriesDataParallel;
    bool getInput;
    ...
    if(getInput){
        ...
        const vector<int> nThreadsOptions = {1, 2, 4, 6, 8, 10, 12};       // Parametri su cui eseguire gli esperimenti
        ...
        for(auto nThreads : nThreadsOptions){
            ...
            if(nThreads != nThreadsOptions.back()) {
                timeseriesDataParallel.timestamps.clear();
                timeseriesDataParallel.timestamps.shrink_to_fit();
                timeseriesDataParallel.values.clear();
                timeseriesDataParallel.values.shrink_to_fit();
            }
        }

        string outputRsultsFile = "speedup_results_SoA.csv";
        ofstream resultsFile(outputRsultsFile);        // Creo un file CSV su cui salvare i risultati
