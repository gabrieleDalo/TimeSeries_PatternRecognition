...
void findBestMatchSequential(const TimeSeriesData& timeseries, const vector<double>& target_sequence, vector<double>& bestMatchingValues, const bool printOn){
    ...
    for(vector<double>::size_type i = 0; i <= timeseries.values.size() - window_size; i++){
        ...
        if(sad < min_sad){
            ...
            for(size_t j = 0; j < window_size; j++){
                bestMatchingValues.push_back(timeseries.values[i + j]);
            }
            bestMatching_initialDate = timeseries.timestamps[i];
            bestMatching_finalDate = timeseries.timestamps[i+window_size-1];
        }
    }
...
void findBestMatchParallel(const TimeSeriesData& timeseries, const vector<double>& target_sequence, vector<double>& bestMatchingValues, const int nThreads, const bool printOn){
    ...
    const size_t data_size = timeseries.values.size();               // Dimensione della timeseries
    ...
    shared(timeseries, target_sequence, window_size, data_size, global_min_sad, bestMatchingValues, bestMatching_initialDate, bestMatching_finalDate, nCheck)
    {
        ...
        for(vector<double>::size_type i = 0; i <= (data_size - window_size); i++){
            ...
            if(sad < local_min_sad){
                ...
                for(size_t j = 0; j < window_size; j++){
                    local_bestMatchingValues.push_back(timeseries.values[i + j]);
                }
                local_bestMatching_initialDate = timeseries.timestamps[i];
                local_bestMatching_finalDate = timeseries.timestamps[i + window_size - 1];
            }
        }
...
void findBestMatchParallelExplicitChunk(const TimeSeriesData& timeseries, const vector<double>& target_sequence, vector<double>& bestMatchingValues, const int nThreads, const bool printOn){
    string bestMatching_initialDate, bestMatching_finalDate;
    double global_min_sad = std::numeric_limits<double>::infinity();
    const size_t window_size = target_sequence.size();
    const size_t data_size = timeseries.values.size();
    int nCheck = 0;
    bestMatchingValues.clear();
...
void findBestMatchParallelExplicitChunkAtomic(const TimeSeriesData& timeseries, const vector<double>& target_sequence, vector<double>& bestMatchingValues, const int nThreads, const bool printOn){
    string bestMatching_initialDate, bestMatching_finalDate;
    double global_min_sad = std::numeric_limits<double>::infinity();
    const size_t window_size = target_sequence.size();
    const size_t data_size = timeseries.values.size();
