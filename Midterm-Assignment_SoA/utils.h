...
inline void acquireSampleTimeSeries(std::vector<double>& sequence, int length, const TimeSeriesData& timeseries){
    sequence.reserve(length);
    ...
    std::transform(timeseries.values.end() - length, timeseries.values.end(), sequence.begin(),
                       [perturbation](double value) {
...
// Legge una timeseries da un file CSV
inline void readCSV(TimeSeriesData& timeseries, const std::string& filename){
    ...
    if(!file.is_open()){
        ...
        while(getline(file, line)){
            ...
            timeseries.values.push_back(stod(valueStr));
    ...
    std::cout << "N° of total timestamps: " << nData << std::endl;
    std::cout << "Reading time (sequential): " << readingTime << " s" << std::endl;
    std::cout << std::endl;
}

// Legge una timeseries da un file CSV in modo parallelo
inline void readCSVParallel(TimeSeriesData& timeseries, const std::string& filename, const int nThreads) {
    ...
    if(!file.is_open()){
        ...
        for(size_t i = 0; i < nLines; ++i){
            if(!timestamps[i].empty() && !std::isnan(values[i])){                       // Filtra i valori non validi
                timeseries.timestamps.push_back(timestamps[i]);
                timeseries.values.push_back(values[i]);
    ...
    std::cout << "N° of total timestamps: " << timeseries.timestamps.size() << std::endl;
...
inline bool checkTimeseriesEquality(TimeSeriesData& timeseries, TimeSeriesData& timeseries2){
    if(timeseries.timestamps.size() != timeseries2.timestamps.size()){
        return false;
    }

    // Confronta gli elementi dei vettori uno per uno
    for(std::vector<double>::size_type i = 0; i < timeseries.timestamps.size(); i++){
        if((timeseries.timestamps[i] != timeseries2.timestamps[i]) || (timeseries.values[i] != timeseries2.values[i])){
            return false;
        }
    }
...
double calculateSAD(const TimeSeriesData& timeseries, const std::vector<double>& target_sequence, size_t start);
