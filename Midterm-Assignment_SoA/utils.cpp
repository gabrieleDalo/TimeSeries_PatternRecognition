...
double calculateSAD(const TimeSeriesData& timeseries, const vector<double>& target_sequence, size_t start){
    double sad = 0.0;
    ...
    for(vector<double>::size_type j = 0; j < target_sequence.size(); j++){
        sad += abs(timeseries.values[start + j] - target_sequence[j]);
    }
    return sad;
}